...
struct CategoryChangesTests {
    ...
    func hasChangesAndIsEmptyBehavior() {
        let testCases: [(changes: CategoryChanges, hasChanges: Bool, description: String)] = [
            ...
            (.init(deletedCategories: ["formal"]), true, "deleted only"),
            (.init(changedCategories: ["summer"]), true, "changed only"),
            (
                .init(
                    newCategories: ["casual"], deletedCategories: ["formal"],
                    changedCategories: ["summer"]), true, "mixed changes"
            ),
        ]
